...
function getKpiSectionHtml(html: string): string {
  const startToken = '<div id="sec-kpi" class="sec">';
  const start = html.indexOf(startToken);
  if (start === -1) return "";

  const nextSection = html.indexOf("<!-- ═══", start + startToken.length);
  if (nextSection !== -1) {
...
const KPI_SECTION_HTML: string = getKpiSectionHtml(STATIC_HTML);
...
function parseTeamRolesFromCards(kpiHtml: string): TeamRole[] {
  const teamStartToken = '<div class="stitle"><span class="snum sn-g">فريق</span>';
  const teamStart = kpiHtml.indexOf(teamStartToken);
  if (teamStart === -1) return [];

  const teamHtml = kpiHtml.slice(teamStart);
  const cardStarts: number[] = [];
  let idx = 0;
  while (true) {
    const found = teamHtml.indexOf('<div class="card">', idx);
    if (found === -1) break;
    cardStarts.push(found);
    idx = found + 1;
  }
  ...
  const cards = cardStarts.map((start, i) => teamHtml.slice(start, cardStarts[i + 1] ?? teamHtml.length));

  return cards.map((cardHtml) => {
    const rawLabel = stripCell(cardHtml.match(/<div class="slbl">([\s\S]*?)<\/div>/)?.[1] ?? "");
    const [namePart, rolePart] = rawLabel.split("—").map((x) => x.trim());
    const bodyHtml = cardHtml.match(/<div style="font-size:12\.5px[\s\S]*?">([\s\S]*?)<\/div>/)?.[1] ?? "";
    const tasks = bodyHtml
      .split(/<br\s*\/?>/g)
      .map((line) => stripCell(line).replace(/^•\s*/, ""))
      .filter(Boolean);

    return {
      name: namePart || rawLabel,
      role: rolePart ?? "",
      tasks,
      kpis: [],
    };
...
const kpiData = (() => {
  const headerTitle = stripCell(
    KPI_SECTION_HTML.match(/<div class="stitle">[\s\S]*?<\/span>\s*([\s\S]*?)<\/div>/)?.[1] ?? "",
  );
  const headerSub = "";
  const tags: string[] = [];
  const accountingPrinciple = "";

  const firstKpiCardsMatch = KPI_SECTION_HTML.match(
    /<div class="g2" style="margin-bottom:18px">([\s\S]*?)<\/div>\s*<div class="card" style="background:rgba\(212,168,83,0\.06\)/,
  );
  const objectiveRows = firstKpiCardsMatch ? parseObjectiveCards(firstKpiCardsMatch[1]) : [];

  const team = parseTeamRolesFromCards(KPI_SECTION_HTML);

  const budgetTableHtml =
    KPI_SECTION_HTML.match(/<div class="tw th-g"><table>[\s\S]*?<\/table><\/div>/)?.[0] ?? "";
  const budgetTable = budgetTableHtml ? parseSimpleTable(budgetTableHtml) : { headers: [], rows: [] };

  const stats: StatCard[] = [];
  const saTable: SimpleTable = { headers: [], rows: [] };
  const egTable: SimpleTable = { headers: [], rows: [] };
  const comparisonTable: SimpleTable = { headers: [], rows: [] };
  const meetingTable: SimpleTable = { headers: [], rows: [] };
  const reportsTable: SimpleTable = budgetTable;
  const marketStats: { infoText: string; cards: MarketCard[] } = { infoText: "", cards: [] };
...
export default function MarketingPage() {
  ...
  return (
    <div className="p-6 space-y-6">
      <div className="flex flex-wrap items-start justify-between gap-3">
        <div>
          <h1 className="text-xl font-bold text-foreground">
            Modonty — {kpiData.headerTitle || "KPI والفريق"}
          </h1>
        ...
        <a
          href="/admin/marketing/modony-plan/static"
          target="_blank"
      ...
      <Card className="border-sky-500/35 bg-sky-500/10">
        <CardContent className="pt-5">
          <p className="text-sm text-sky-100">
            <span className="font-bold text-sky-300">💡 تلميح:</span> المهندس خالد مش فاضي — يتم توزيع المهام على
            الفريق المختص لإدارة الحساب الشخصي للمهندس خالد. توضيح للفريق: المقصود هو حساب LinkedIn الشخصي
            (وليس صفحة الشركة).
          </p>
        </CardContent>
      </Card>
